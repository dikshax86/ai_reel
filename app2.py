...
possible_paths = [
    "D:\Dikha_workspace\Dikha_workspace\videodb-reel-main\project\ImageMagick-7.1.1-47-Q16-HDRI-x64-dll.exe"
]
...
# <<< REVISED FUNCTION - Replace your existing focus_on_speaker with this >>>

# <<< REVISED FUNCTION V3 - Replace your existing focus_on_speaker with this >>>

# <<< PASTE THIS ENTIRE FUNCTION INTO YOUR app2.py FILE, REPLACING THE OLD ONE >>>

def focus_on_speaker(clip, target_width=1080, target_height=1920):
    """
    Processes video using an intelligent "Virtual Director" with state memory,
    proper framing, audio-visual scoring, and robust fallback logic.
    """
    # --- Director Tuning Parameters ---
    ...
    frame_state = {
        ...
        # <<< FIX: Enhanced state management for stability >>>
        'speaker_data': {},
        'active_speaker_id': None,
        'last_valid_target_center': None, # <<< KEY FIX: Remembers the last good target to prevent focusing on objects
        'smoothing_factor': 0.85,
        
        'current_shot_type': 'single_speaker',
        'current_pan_center': None,
        'last_shot_decision_time': -1,
        'time_in_current_state': 0,
        'is_in_transition': False,
    ...
    # <<< FIX: Robust framing logic to prevent shoulder cut >>>
    def render_single_speaker_view(frame, center_pos):
        h, w = frame.shape[:2]
        crop_height = h
        crop_width = int(crop_height * (9/16))

        # Calculate the top of the crop window to give the speaker "headroom"
        # We want the person's face to be in the upper third of the final shot.
        face_y = center_pos[1]
        
        # desired_y_in_frame is where we want the face to be vertically in the final crop (e.g., 30% from the top)
        desired_y_in_frame = crop_height * 0.30
        top = int(face_y - desired_y_in_frame)

        # Center horizontally
        left = int(center_pos[0] - crop_width / 2)

        # Boundary checks to ensure the crop is within the original frame
        left = max(0, min(left, w - crop_width))
        top = max(0, min(top, h - crop_height))
    ...
    def process_frame(get_frame, t):
        ...
        if frame_state['current_pan_center'] is None:
            initial_center = (w / 2, h / 2)
            frame_state['current_pan_center'] = initial_center
            frame_state['last_valid_target_center'] = initial_center

        # --- 1. Audio and Visual Data Acquisition ---
        frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        
        # Get audio for this frame time 't'
        audio_frame = clip.audio.get_frame(t)
        if audio_frame.ndim > 1:
            audio_frame = np.mean(audio_frame, axis=1) # Convert to mono

        # Use your existing audio analysis function
        audio_score = detect_active_speaker(frame, audio_frame, sample_rate=clip.audio.fps)
        ...
        if face_results.multi_face_landmarks:
            for i, landmarks in enumerate(face_results.multi_face_landmarks):
                ...
                # Smooth the position
                if face_id in frame_state['speaker_data'] and 'position' in frame_state['speaker_data'][face_id]:
                    alpha = frame_state['smoothing_factor']
                    prev_pos = frame_state['speaker_data'][face_id]['position']
                    smoothed_pos = (prev_pos[0] * alpha + raw_center[0] * (1-alpha), prev_pos[1] * alpha + raw_center[1] * (1-alpha))
                else:
                    smoothed_pos = raw_center

                lip_movement, _ = detect_lip_movement(landmarks)
                
                # <<< FIX: Combined audio-visual score >>>
                # Weight lip movement more heavily as it's specific to a face
                combined_score = (lip_movement * 0.7) + (audio_score * 0.3)
        ...
        frame_state['speaker_data'] = detected_faces
        num_people = len(detected_faces)
        
        # --- 2. Director's State Machine (Split-Screen Logic) ---
        # This logic remains as it was, it's working well.
        previous_shot_type = frame_state['current_shot_type']
        new_shot_type = previous_shot_type
        dt = t - frame_state['last_shot_decision_time'] if frame_state['last_shot_decision_time'] > -1 else 0
        frame_state['last_shot_decision_time'] = t
        ...
        if new_shot_type != previous_shot_type:
            frame_state['time_in_current_state'] = 0
            frame_state['current_shot_type'] = new_shot_type
            if frame_state['last_stable_frame'] is not None:
                frame_state['is_in_transition'] = True
                frame_state['transition_progress'] = 0.0
                frame_state['transition_from_frame'] = frame_state['last_stable_frame']

        # --- 3. Intelligent Speaker Selection & Target Acquisition ---
        new_target = None
        if frame_state['current_shot_type'] == 'single_speaker' and num_people > 0:
            # Find person with the highest activity score
            best_speaker_id = max(frame_state['speaker_data'], key=lambda id: frame_state['speaker_data'][id]['score'])
            max_score = frame_state['speaker_data'][best_speaker_id]['score']
            
            talking_threshold = 0.15 # Tweak this if needed
            if max_score > talking_threshold:
                # We have a clear speaker
                frame_state['active_speaker_id'] = best_speaker_id
                new_target = frame_state['speaker_data'][best_speaker_id]['position']
            elif frame_state['active_speaker_id'] in frame_state['speaker_data']:
                # No one is clearly talking, so stay on the last active speaker if they're still visible
                new_target = frame_state['speaker_data'][frame_state['active_speaker_id']]['position']
            else:
                # Last active speaker is gone, just pick the first available person
                new_target = frame_state['speaker_data'][list(frame_state['speaker_data'].keys())[0]]['position']

        # <<< CORE FIX: Update the camera's memory and pan target >>>
        if new_target:
            # We found a person, so we update our "memory" of a valid target
            frame_state['last_valid_target_center'] = new_target
        
        # The camera always aims for the last valid target, preventing it from ever aiming at the table
        target_pan_center = frame_state['last_valid_target_center']
            
        # Smoothly pan the camera towards the target
        current_pos = np.array(frame_state['current_pan_center'])
        target_pos = np.array(target_pan_center)
        # Use a slightly slower pan for a more cinematic feel
        ...
        if frame_state['current_shot_type'] == 'single_speaker':
            target_frame = render_single_speaker_view(frame, frame_state['current_pan_center'])
        elif frame_state['current_shot_type'] == 'split_screen' and len(frame_state['speaker_data']) >= 2:
            # Ensure keys 0 and 1 exist before accessing
            if 0 in frame_state['speaker_data'] and 1 in frame_state['speaker_data']:
                p1_pos = frame_state['speaker_data'][0]['position']
                p2_pos = frame_state['speaker_data'][1]['position']
                target_frame = create_split_screen(frame, p1_pos, p2_pos, target_width, target_height)

        # Final safety net
        if target_frame is None:
            target_frame = render_single_speaker_view(frame, frame_state['current_pan_center'])

        # --- 5. Apply Cross-Fade Transition (No changes needed here) ---
        final_frame = target_frame
        if frame_state['is_in_transition'] and frame_state['transition_from_frame'] is not None:
            if frame_state['transition_from_frame'].shape != target_frame.shape:
                frame_state['is_in_transition'] = False
                final_frame = target_frame
            else:
                frame_state['transition_progress'] += 1 / (TRANSITION_DURATION * fps)
                alpha = min(frame_state['transition_progress'], 1.0)
                final_frame = cv2.addWeighted(frame_state['transition_from_frame'], 1 - alpha, target_frame, alpha, 0)
                if alpha >= 1.0:
                    frame_state['is_in_transition'] = False

        if not frame_state['is_in_transition']:
            frame_state['last_stable_frame'] = final_frame.copy()
